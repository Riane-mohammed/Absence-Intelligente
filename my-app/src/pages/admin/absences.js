...
export const AbsencesPage = () => {
    ...
    const filterAbsencesByClass = (class_) => {
        const filtered = allAbsences.filter(absence => absence.class.toLowerCase() === class_.toLowerCase());
        setTotalCount(filtered.length);
        const start = (currentPage - 1) * itemsPerPage;
        const end = start + itemsPerPage;
        const paginatedData = filtered.slice(start, end);
        setAbsences(class_ ? paginatedData : allAbsences.slice(start, end));
    };
    ...
    const searchByFullName = (search) => {
        ...
        const end = start + itemsPerPage;
        const paginatedData = filtered.slice(start, end);
        setAbsences(search ? paginatedData : allAbsences.slice(start, end));
    };
